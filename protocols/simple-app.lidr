## A simple app {.allowframebreaks}

* In this section we build on top of the implementation of the
`Database` protocol we just created.

* Before we were interested, essentially, on specifying a datatype
  that captures the _behavior_ (or automaton) of the protocol,
  guaranteeing that no computation (or transition) takes place without
  fulfilling the contract (pre and postconditions) of each
  computation.
  
* Now we wish to write a _running_ application that can, with proper
  extensions, become an actual web system, when the DBMS primitives
  are replaced by calls to the DBMS' API.
