...
now programming with _pattern matching_. And what is it? Simply define
a function by _cases_. When we hit an instance of `Vect`, how does it
look like? It is either the empty vector, built with constructor
`Nil`, or a non-empty vector, built using operator `::`. These two
...
Idris' standard library.) Note that the type of `[(1, "a"), (2, "b"),
...
types of the elements of the vectors being zipped. That is, the `Vect`
