...
As functions:
\begin{eqnarray*}
\mathtt{bhask}(a,b,c) & = & \left( -b + \sqrt{\mathtt{delta}(a,b,c)} / 2a \mathbf{,} 
-b - \sqrt{\mathtt{delta}(a,b,c)} / 2a\right) \\
\mathtt{delta}(a,b,c) & = & b^2 - 4acb
\end{eqnarray*}

## First attempt: no types.
...
## Second attempt: still no types.

Now, assuming we are instered only on Real results, how should `bhask`
  deal with the possilibity of a negative `delta`?
...
```idris
Wed Jul 31@14:22:57:sc$ idris intro.lidr
     ____    __     _
    /  _/___/ /____(_)____
